...
In the former case, the TSM can enforce that TVM is allowed to execute and
release boot key. In the latter, it can provide continuous access to secrets as
part of a licensing model.
====
...
and integrity protected using a symmetric key encrypted inside lockboxes.
...
implementation-specific encoding format, such as Concise Binary Object
...
tap-header = {
    ...
    tap-header-version: int .ge 0 .le 65535,
...
under which a TVM can execute and access the secrets. The TSM must not expose
secrets to a TVM, if any condition fails.

[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "TAP Attributes"]
[source,cddl]
----
tap-attribute-key = int {
   0 => hardware_serial_number,
   1 => firmware_build_time,
   2 => license_expiration,
}
...
===== Secrets
The `secrets` section contains a map of secrets where a secret could be, for
example, a cryptographic key or an SSH key. Each secret is identified by a
unique number. The TVM can retrieve secrets from the TSM using the
`sbi_covg_retrieve_secret()` ABI.
