...
During the TVM creation, the TSM must interact with the local verifier to
authorize TVM execution, providing a TAP supplied by the OS/VMM as part of the
TVM creation request. During the TVM runtime, the TVM presents an attestation
results (e.g., a signed attestation token) to the local secret broker to
retrieve the corresponding secrets.
...
6 bytes long header that precedes the CBOR-encoded payload.
...
representations. This design avoids partial CBOR parsing prior to buffer
...
at rest by encrypting the VM's disk with a secret key that the TVM obtains
during boot following successful attestation. In such a case, the TAP would
contain a disk encryption secret that the TVM firmware could retrieve from TSM.
====
