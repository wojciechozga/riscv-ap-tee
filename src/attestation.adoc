...
Systems that cannot rely on remote attestation due to unstable or unavailable
network connectivity may instead use local attestation.
In this model, the TSM attests to the integrity of a TVM and conditions under
which it executes using the rules defined in a TVM attestation payload (TAP).
A TAP is a cryptographically protected file created by an authorized entity,
such as a TVM creator. A TAP file acts as a source of truth for
authorization decisions in environments with unreliable network.
...
Local attestation introduces two components, a local verifier and a local secret
broker. The local verifier attests to the integrity of a TVM and authorizes its
execution. The local secret broker provisions secrets to a TVM. Both components
rely on a TAP as a source of truth for authorizing TVM's actions. In a local
attestation flow, the TSM interacts with the local verifier to authorize
TVM execution, supplying a TAP provided by the OS/VMM. To obtain secrets,
a running TVM first requests evidence (e.g., a signed attestation token)
from the local verifier, and then presents this evidence to the local secret
broker to retrieve the corresponding secrets.

[NOTE]
====
The local verifier and secret broker roles are defined as distinct logical
components to enable a broader range of deployment models and use cases.
For example, an implementation may realize the secret broker within a TVM,
a platform RoT, or an external centralized computing node used by many devices.
In other implementations, a TSM may consolidate both the local verifier and secret
broker roles within a single component.
====
...
the platform.
. TVM Authorization: The system enforces that only TVMs created by a trusted
entity run on the platform.
...
Access to such key should be linked to the integrity of all TCB elements, such
as the TSM and other M-mode firmware.
...
A CoVE-enabled platform that supports the TVM authorization must implement the
...
The creator of a TVM must sign the TAP associated with the TVM with the
corresponding private key. During the TVM creation, the TSM requests the local
...
The TAP carries information that allows the local verifier to authorize the
TVM's execution on the platform and the local secret broker to provision the
...
tap-body-mac = COSE_Mac0                              ; The plaintext is defined as tap-body-mac
...
the TAP. The header must be always 4 bytes aligned, 10 bytes long, and be serialized
in a binary format.
...
generated random symmetric key (TSK).
...
if any condition fails. A presence of an attribute that is not supported causes
the verification condition to fail.
