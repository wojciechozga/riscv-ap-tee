...
broker. The local verifier attests to the TVM's integrity and authorizes the TVM's
execution. The local secret broker provides a TVM with secrets. Both components
use a TAP that contains the specification of reference integrity measurements,
secrets, and conditions under which a TVM can execute and access secrets.
...
tap-body-encrypt = COSE_Encrypt1                      ; The plaintext is defined as tap-body-encrypt

tap = (
   tap-header,
   tap-lockboxes,
   tap-body-mac / tap-body-encrypt
)
----
